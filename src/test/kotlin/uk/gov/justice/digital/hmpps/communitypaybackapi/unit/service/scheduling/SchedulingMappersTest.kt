...
class SchedulingMappersTest {
  ...
  inner class NDSchedulingAllocationToSchedulingAllocation {
    ...
    fun `all fields populated`() {
      ...
      assertThat(result.pickUpLocationCode).isEqualTo("pickUpCode")
    }
  ...
  inner class SchedulingRequiredAppointmentToNDCreateAppointment {
    ...
    fun `map all fields`() {
      ...
      assertThat(result.pickUp?.time).isEqualTo(LocalTime.of(13, 14))
      assertThat(result.pickUp?.location?.code).isEqualTo("pickUpLocCode")
    }
  }
}
