...
import uk.gov.justice.digital.hmpps.communitypaybackapi.common.validation.Validator
import uk.gov.justice.digital.hmpps.communitypaybackapi.factory.random
import java.time.LocalDate

class ValidatorTest {
  class TestValidator : Validator<TestData>() {
  ...
  inner class Validate {
    ...
    fun `should report warnings when warning rules are not satisfied`() {
      val validator = TestValidator()
      val value = TestData.valid().copy(startDate = LocalDate.now(), endDate = LocalDate.now().plusDays(14), isWeeklyEvent = true)
    ...
    fun `should ignore rules when assumptions are not satisfied`() {
      val validator = TestValidator()
      val value = TestData.valid().copy(isWeeklyEvent = false)
        .copy(startDate = LocalDate.now(), endDate = LocalDate.now().plusDays(14))
        ...
        object : Validator<TestData>() {
          override fun configureRules() {
            rule {
              expect { value -> value.notes != null }
              otherwise {
                this isError "MISSING_EXTRA_DATA"
                field = "notes"
              }
            }
          }
