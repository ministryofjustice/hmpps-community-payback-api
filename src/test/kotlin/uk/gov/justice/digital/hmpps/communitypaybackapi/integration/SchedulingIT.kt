...
/*
Currently these tests invoke scheduling directly. once the code is fully integrated
they should instead invoke scheduling via appointment changes
 */
class SchedulingIT : IntegrationTestBase() {
  ...
  inner class AppointmentUpdate {
    ...
    fun `UPDATE-01 New Truncated Appointment created after start time amended and outcome recorded`() {
      ...
      val schedulingDate = LocalDate.now().findNextDateForDayOfWeek(DayOfWeek.WEDNESDAY)
        ...
        NDUnpaidWorkRequirement(
          ...
          appointments = listOf(
            NDSchedulingAppointment.validWithOutcome().copy(
              ...
              endTime = LocalTime.of(18, 0),
              timeCredited = Duration.ofHours(4),
            ),
            NDSchedulingAppointment.validWithOutcome().copy(
            ...
            NDSchedulingAppointment.validWithOutcome().copy(
              ...
              startTime = LocalTime.of(10, 0),
              endTime = LocalTime.of(18, 0),
              timeCredited = Duration.ofHours(8),
            ),
            NDSchedulingAppointment.validWithOutcome().copy(
            ...
            NDSchedulingAppointment.validWithOutcome().copy(
              ...
              timeCredited = Duration.ofHours(7),
            ),
          ),
        ),
      )
      ...
      schedulingService.schedulePlanAndApply(CRN, EVENT_NUMBER, SchedulingTrigger.AppointmentUpdate)
  ...
  private fun LocalDate.findNextDateForDayOfWeek(dayOfWeek: DayOfWeek): LocalDate = with(TemporalAdjusters.nextOrSame(dayOfWeek))
}
