...
data class ProjectAllocationDto(
  ...
  @param:Schema(description = "Number of hours allocated", example = "40")
  val hours: Int,
)
...
class ProjectController(val projectService: ProjectService) {
  ...
  fun getProjectAllocations(
    @Parameter(description = "Start date in format dd/MM/yyyy", example = "01/09/2025", required = true)
    @RequestParam
    @DateTimeFormat(pattern = "d/M/yyyy") startDate: LocalDate, // ← Now LocalDate

    @Parameter(description = "End date in format dd/MM/yyyy", example = "07/09/2025", required = true)
    @RequestParam
    @DateTimeFormat(pattern = "d/M/yyyy") endDate: LocalDate, // ← Now LocalDate
